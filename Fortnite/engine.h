...
uintptr_t GameBaseAddress = (uintptr_t)LI_FN(GetModuleHandleA)(xorstr("FortniteClient-Win64-Shipping.exe"));
...
static inline auto SpoofCall(Ret(*fn)(Args...), Args... args) -> Ret
{
	...
	if (!jmprbx) {
		const auto ntdll = reinterpret_cast<const unsigned char*>(GameBaseAddress);
...
namespace SDK
{
	...
	namespace Classes
	{
		...
		public:
			...
			{
				auto GetFOVAngle = reinterpret_cast<float(*)(UINT64, char*)>(*(ULONG_PTR*)(*(ULONG_PTR*)PlayerCameraManager + 0x780));
			...
			{
				auto GetCameraLocation = reinterpret_cast<Vector3(*)(UINT64, char*)>(*(ULONG_PTR*)(*(ULONG_PTR*)PlayerCameraManager + 0x7C8));
			...
			{
				auto GetCameraRotation = reinterpret_cast<Vector3(*)(UINT64, char*)>(*(ULONG_PTR*)(*(ULONG_PTR*)PlayerCameraManager + 0x7C0));
			...
			static bool GetPlayerViewPoint(uintptr_t PlayerController, Vector3* vCameraPos, Vector3* vCameraRot)
			{
				...
				if (!pGetPlayerViewPoint)
				{
					...
					pGetPlayerViewPoint = *(uintptr_t*)(VTable + 0x7B0);
